...
export class ManualReviewMiddleware {
  ...
  static sanitize(findings: Finding[]): Finding[] {
    return findings.map((finding) => {
      ...
      // If it's a visual item, we force it to Manual status for human confirmation.
      if (this.MANUAL_FRONTEND_IDS.includes(idNumber)) {
        return {
          ...finding,
          status: "Manual",
        };
